...
public class Game extends Constants {
    ...
    static AlienExplosion explosion = new AlienExplosion();
    static CannonBang bang = new CannonBang();
       ...
       if (gameOver)
           for (int y = 0; y < GAME_OVER.length; y++)
               for (int x = 0; x < GAME_OVER[y].length; x++)
                   if (GAME_OVER[y][x] == 1) g.fillRect(x*BLOCK_SIZE + 170, y*BLOCK_SIZE + 250, BLOCK_SIZE, BLOCK_SIZE);
